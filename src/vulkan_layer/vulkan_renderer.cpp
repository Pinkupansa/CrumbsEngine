...
VulkanRenderer::VulkanRenderer(GLFWwindow* _window, uint32_t _width,
                               uint32_t _height)
    : window(_window),
      width(_width),
      height(_height),
      instance(window),
      device(instance),
      mainSurface(instance, device, window),
      swapchain(device, mainSurface),
      textureBundle(device, ATLAS_SIZE),
      alignment(device.getProperties().limits.minUniformBufferOffsetAlignment),
      uboAlignedSize((uboSize + alignment - 1) & ~(alignment - 1)),

      vertexBuffer(device, VulkanBufferType::Vertex,
                   MAX_VERTEX_NUMBER * sizeof(Vertex), nullptr, false, 0,
                   "Vertex Buffer"),
      indexBuffer(device, VulkanBufferType::Index,
                  MAX_INDEX_NUMBER * sizeof(uint32_t), nullptr, false, 0,
                  "Index Buffer"),

      allObjectsUB(device, VulkanBufferType::Uniform,
                   MAX_OBJECTS_UB * uboAlignedSize, nullptr, true,
                   uboAlignedSize, "Objects UB"),
      sceneDataUB(device, VulkanBufferType::Uniform,
                  MAX_SCENE_DATA * sizeof(VulkanSceneUBO), nullptr, false, 0,
                  "SceneData UB"),

      allObjectsUBDescriptor(
          device, allObjectsUB,
          VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, uboSize,
          "Objects UB Descriptor Set"),
      sceneDataUBDescriptor(
          device, sceneDataUB,
          VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
          sizeof(VulkanSceneUBO), "Scene UB Descriptor Set"),
      shadowMap(device, SHADOW_MAP_SIZE, SHADOW_MAP_SIZE,
                DEFAULT_SHADOW_FORMAT),
      shadowImageDrawer(device, shadowMap.getShadowAttachment()->getExtent(),
                        shadowMap.getAttachmentsPerFrameBuffer(), true,
                        ...
                        {shadowShaderPath}, {}, VK_CULL_MODE_NONE, true, true,
                        false, VulkanAlphaBlendMode::None, "Shadow Image Drawer") {
  /*TODO : Create instead one image drawer per fragShader, and one objectsUB
   * per shader ?*/
  std::cout << "Vertex buffer size: " << MAX_VERTEX_NUMBER * vertexSize
            << std::endl;
  std::cout << "Index buffer size: " << MAX_INDEX_NUMBER * indexSize
            << std::endl;
  std::cout << "Objects UB size: " << MAX_OBJECTS_UB * uboAlignedSize
            << std::endl;
...
void VulkanRenderer::addMeshDrawCall(int shaderIndex, FSBObject fsbObject,
                                     uint32_t meshIndex, glm::mat4 transform,
                                     bool castsShadows) {
  if (shaderIndex >= imageDrawersPerFragShader.size()) {
    Debug::LogWarning("Shader index " + std::to_string(shaderIndex) +
                      " does not exist !");
  }
...
void VulkanRenderer::initSceneData(const Scene& scene) {
  glm::mat4 proj =
      glm::perspective(glm::radians(80.0f),
                       ...
                       0.1f, 500.0f);
  proj[1][1] *= -1;
  sceneData = VulkanSceneUBO(scene.getMainCamera()->getView(), proj,
                             scene.lightDir, scene.lightColor,
                             scene.groundColor, scene.skyColor);
...
void VulkanRenderer::drawFrame() {
  ...
  shadowImageDrawer.draw(vertexBuffer, indexBuffer, meshPool,
                         drawCallMeshIndices);
  ...
  for (int i = 0; i < imageDrawersPerFragShader.size(); i++) {
    ...
    imageDrawersPerFragShader[i]->draw(
...
VulkanRenderTexture* VulkanRenderer::createRenderTexture(std::string name) {
...
int VulkanRenderer::loadShader(const VulkanShaderData& shaderData) {
  ...
  VulkanBuffer* objectUBForShader = new VulkanBuffer(
      device, VulkanBufferType::Uniform, MAX_OBJECTS_UB * uboAlignedSize,
      nullptr, true, uboAlignedSize, "Shader " + filePath + " ObjectUB");

  objectsUBPerFragShader.push_back(objectUBForShader);

  VulkanUBDescriptor* objectUBDescForShader = new VulkanUBDescriptor(
      device, *objectsUBPerFragShader.back(),
      VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, uboSize,
      "Shader " + filePath + "Objects UB Descriptor Set");
  ...
  VulkanUBDescriptor* customUBDescForShader = new VulkanUBDescriptor(
      device, flexibleBufferPerFragShader.back()->getBuffer(),
  ...
  std::vector<VulkanDescriptorData> descriptorsForShader = {
      ...
      customUBDescForShader->getDescData(
          0, DEFAULT_CUSTOM_FRAG_PROPERTIES_SET_NUMBER)};
  ...
  if (shaderData.colorAttachment == nullptr) {
    ...
    shaderDrawer = new VulkanImageDrawer(
        ...
        swapchain.getAttachmentsPerFrameBuffer(), !loadedFirstDrawer,
        descriptorsForShader, swapchain.getSyncObjects(),
        swapchain.getFenceResetCallback(),
    ...
    if (shaderData.colorAttachment->getType() != VulkanAttachmentType::Color) {
      throw std::runtime_error("Non-color custom attachment type not supported !");
    }
    shaderDrawer = new VulkanImageDrawer(
        device, mainSurface.getCapabilities().currentExtent,
...
void VulkanRenderer::buildTextureAtlas() { textureBundle.buildTextureAtlas(); }
