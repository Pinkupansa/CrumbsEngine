...
UniformVariant createDefaultUniform(const std::string& typeName) {
  ...
  if (typeName == "uint") return uint32_t(0);
  if (typeName == "int") return int(-1);
  if (typeName == "vec2") return glm::vec2(1.0f);
  if (typeName == "vec3") return glm::vec3(0.0f);
  if (typeName == "vec4") return glm::vec4(0.0f);
  if (typeName == "mat2") return glm::mat2(1.0f);
  if (typeName == "mat3") return glm::mat3(1.0f);
  if (typeName == "mat4") return glm::mat4(1.0f);
...
uint64_t getStd140Alignment(const UniformVariant& v) {
  switch (v.index()) {
    ...
    case 8:
    case 9:
      return 16;  // mat2, mat3, mat4 (columns aligned to vec4)
    default:
      throw std::runtime_error("Unknown UniformVariant type");
  }
}
...
uint64_t getStd140Size(const UniformVariant& v) {
  switch (v.index()) {
    ...
    case 4:
      return 8;
    case 5:
      return 16;  // vec3 padded to 16
    case 6:
      return 16;  // vec4
    case 7:
      return 16 * 2;  // mat2: 2 vec4 columns
    case 8:
      return 16 * 3;  // mat3: 3 vec4 columns
    case 9:
      return 16 * 4;  // mat4: 4 vec4 columns
    default:
      throw std::runtime_error("Unknown UniformVariant type");
  }
}
...
void debugPrintFSBObject(const FSBObject& obj) {
  ...
  for (const auto& [name, value] : obj) {
    ...
    std::cout << "  " << name << " : " << uniformVariantTypeName(value)
              << " | align=" << align << " | size=" << getStd140Size(value)
              << " | offset=" << offset << " | value=";
