...
VulkanImageDrawer::VulkanImageDrawer(
    ...
    : pDevice(device),
      renderTargetSyncObjects(renderTargetSyncObjects),
      renderTargetFenceResetCallback(renderTargetFenceResetCallback),
      descriptors(descriptors),
      isFirstPass(isFirstPass),
      renderPass(device, attachmentsPerFramebuffer[0]),
      imageExtent(extent),
      graphicsPipeline(device, renderPass, extent, descriptors,
                       vertShaderPaths, fragShaderPaths, cullMode,
                       enableDepthTest, enableDepthWrite, alphaBlendMode,
                       isFullScreenShader, name + " Pipeline"),
      framebuffers(device, renderPass, attachmentsPerFramebuffer,
                   name + " Framebuffers"),
...
void VulkanImageDrawer::draw(const VulkanBuffer& vertexBuffer,
  ...
  submitInfo.pSignalSemaphores =
      renderTargetSyncObjects.getSignalSemaphore(syncIndex);
