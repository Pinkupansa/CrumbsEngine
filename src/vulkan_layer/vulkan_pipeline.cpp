...
VkPipelineLayout VulkanPipeline::getLayout() const { return pipelineLayout; }
...
VulkanPipeline::VulkanPipeline(
    ...
    const std::vector<std::string>& fragPaths, VkCullModeFlagBits cullMode,
    bool enableDepthTest, bool enableDepthWrite,
    VulkanAlphaBlendMode alphaBlendMode, bool isFullscreen, std::string name)
    : pDevice(device) {
  ...
  for (std::string path : fragPaths) {
    ...
    shaderStages.push_back(
        createFragmentShaderStageCreateInfo(fragShaderModule));
  }
  ...
  VkPipelineVertexInputStateCreateInfo vertexInputInfo =
      isFullscreen ? createFullscreenVertexInputInfo()
                   : createVertexInputInfo(bindingDescription,
  ...
  pipelineLayout =
      createPipelineLayout(device, descLayouts, {}, name + " Layout");

  VkPipelineDepthStencilStateCreateInfo depthStencil =
      createDepthStencilInfo(enableDepthTest, enableDepthWrite);

  pipeline =
      createPipeline(device, shaderStages, vertexInputInfo, inputAssembly,
                     viewportStateInfo, rasterizerInfo, multisamplingInfo,
                     colorBlendingInfo, pipelineLayout, renderPass.getRenderPass(),
                     depthStencil, name);
}
