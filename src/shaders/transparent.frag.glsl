...
void main() {
    ...
    vec4 texColor = customProps.tint*computeTexColor(fragUV, customProps.atlasOffset, customProps.relativeTextureSize, customProps.tilingFactor, fragCamPos.z);

    float alpha = texColor.a;
    
    // Optional: depth-based weight
    float weight = alpha;
    
    outColor  = alpha > 0.1f ? vec4(texColor.rgb * weight, weight) : vec4(0.0);
